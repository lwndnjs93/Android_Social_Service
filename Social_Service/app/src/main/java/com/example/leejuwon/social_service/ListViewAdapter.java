...
public class ListViewAdapter extends BaseAdapter implements Filterable {
    ...
    //Adapter에 추가된 데이터를 저장하기 위한 ArratList. (원본 데이터 리스트)
    ArrayList<ListViewItem> listViewItemList = new ArrayList<ListViewItem>();

    //필터링된 결과 데이터를 저장하기 위한 ArrayList, 최초에는 전체 리스트 보유.
    ArrayList<ListViewItem> filteredItemList = listViewItemList;
    ...
    // 지정한 위치(position)에 있는 데이터와 관계된 아이템(row)의 ID를 리턴. : 팔수 구현
    @Override
    public long getItemId(int position) {
        return position;
    }

    //position에 위치한 데이터를 화면에 출력하는데 사용될 view 리턴. : 필수 구현
    @Override
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //화면에 표시될 View(Layout이 infalte된)으로부터 위젯에 대한 참조 획득
    ...
    public void addItem(String titleStr, String descStr, String a01, String a02, String tel, String subtitle){
