...
public class Basic_Life01_01 extends AppCompatActivity {
    Intent intent;
    int work01, //근로소득
            business01, //사업소득
            property01, //재산소득
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.basic__life01_01);
        intent = getIntent();

        //소득재산 시작
        if (intent.getStringExtra("01").isEmpty()) {
            work01 = 0;
        } else {
            work01 = Integer.parseInt(intent.getStringExtra("01"));
        }//근로소득
        if (intent.getStringExtra("02").isEmpty()) {
            business01 = 0;
        } else {
            business01 = Integer.parseInt(intent.getStringExtra("02"));
        }//사업소득
        if (intent.getStringExtra("03").isEmpty()) {
            property01 = 0;
        } else {
            property01 = Integer.parseInt(intent.getStringExtra("03"));
        }//재산소득
        if (intent.getStringExtra("04").isEmpty()) {
            ect01 = 0;
        } else {
            ect01 = Integer.parseInt(intent.getStringExtra("04"));
        }//기타소득
        //소득재산 끝

        //지출요인 시작
        if (intent.getStringExtra("05").isEmpty()) {
            medical01 = 0;
        } else {
            medical01 = Integer.parseInt(intent.getStringExtra("05"));
        }//월평균 의료비
        if (intent.getStringExtra("06").isEmpty()) {
            rehabilitation01 = 0;
        } else {
            rehabilitation01 = Integer.parseInt(intent.getStringExtra("06"));
        }//재활보조금
        if (intent.getStringExtra("07").isEmpty()) {
            pension01 = 0;
        } else {
            pension01 = Integer.parseInt(intent.getStringExtra("07"));
        }//연금보험료
        //지출요인 끝

        if (intent.getStringExtra("08").isEmpty()) {
            house01 = 0;
        } else {
            house01 = Integer.parseInt(intent.getStringExtra("08"));
        }//주거용 주택

        //일반재산 시작
        if (intent.getStringExtra("09").isEmpty()) {
            building01 = 0;
        } else {
            building01 = Integer.parseInt(intent.getStringExtra("09"));
        }//건축물
        if (intent.getStringExtra("10").isEmpty()) {
            land01 = 0;
        } else {
            land01 = Integer.parseInt(intent.getStringExtra("10"));
        }//토지
        //일반재산 끝

        if (intent.getStringExtra("11").isEmpty()) {
            deposit_for_lease01 = 0;
        } else {
            deposit_for_lease01 = Integer.parseInt(intent.getStringExtra("11"));
        }//임차 보증금
        if (intent.getStringExtra("12").isEmpty()) {
            ect_property01 = 0;
        } else {
            ect_property01 = Integer.parseInt(intent.getStringExtra("12"));
        }//기타재산

        if (intent.getStringExtra("13").isEmpty()) {
            finance_income01 = 0;
        } else {
            finance_income01 = Integer.parseInt(intent.getStringExtra("13"));
        }//금융소득

        if (intent.getStringExtra("14").isEmpty()) {
            car01 = 0;
        } else {
            car01 = Integer.parseInt(intent.getStringExtra("14"));
        }//차량 가액

        if (intent.getStringExtra("15").isEmpty()) {
            debt01 = 0;
        } else {
            debt01 = Integer.parseInt(intent.getStringExtra("15"));
        }//부채

        if (intent.getStringExtra("16").isEmpty()) {
            city = 0;
        } else {
            city = Integer.parseInt(intent.getStringExtra("16"));
        }
        ...
        // 1652931
        if (recognition <= 50) {
            TextView tv01 = (TextView) findViewById(R.id.tv001);
            tv01.setText("중위소득 30% 이하로 생계급여 / 의료급여 / 주거급여 / 교육급여 수급대상자로 선정될 가능성이 있습니다.");
        } else if (50 <= recognition && recognition <= 66) {
            TextView tv01 = (TextView) findViewById(R.id.tv001);
            tv01.setText("중위소득 40% 이하로 의료급여 / 주거급여 / 교육급여 수급대상자로 선정될 가능성이 있습니다.");
        } else if (66 <= recognition && recognition <= 71) {
            TextView tv02 = (TextView) findViewById(R.id.tv001);
            tv02.setText("중위소득 43% 이하로 주거급여 / 교육급여 수급대상자로 선정될 가능성이 있습니다.");
        } else if (71 <= recognition && recognition <= 83) {
            TextView tv02 = (TextView) findViewById(R.id.tv001);
            tv02.setText("중위소득 50% 이하로 교육급여 수급대상자로 선정될 가능성이 있습니다.");
        } else {
            TextView tv02 = (TextView) findViewById(R.id.tv001);
            tv02.setText("생계급여/의료급여/주거급여/교육급여 선정기준 이상으로   \t 지원대상이 아닌 걸로 예상됩니다. ");
        }
    }
